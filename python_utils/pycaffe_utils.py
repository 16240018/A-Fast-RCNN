...
def net_surgery(net, json_file_or_dict):
    ...
    target_blobs = list(net.params.keys()) 
    target_blobs = dict(list(zip(target_blobs, [0]*len(target_blobs))))
    ...
    for src_desc in source_description:
        ...
        for j in range(len(src_desc['copy_ops']['dest'])):
            ...
            assert src_name in list(net_source.params.keys()), \
            ...
            for k in range(len(net.params[dest_name])):
    ...
    unusual = [x for x in list(target_blobs.keys()) if target_blobs[x] is not 1]
    for x in unusual:
